...
export function IncrementTable({ contractAddress }: IncrementTableProps) {
  const [page, setPage] = useState(0);

  const { data } = useQuery({
    queryKey: ["Amp", "Demo", { table: "increments", address: contractAddress, page }] as const,
    async queryFn() {
      /**
      ...
      return await performAmpQuery<Increment>(
        `SELECT block_num, timestamp, count FROM "eth_global/counter@dev".incremented ORDER BY block_num DESC LIMIT ${ITEMS_PER_PAGE} OFFSET ${offset}`
      );
    },
  });
          ...
          <div className="inline-block min-w-full py-2 align-middle">
            <table className="relative min-w-full divide-y divide-gray-300 dark:divide-white/15">
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-white/10">
                {(data ?? []).map((increment, index) => (
                  <tr key={`${increment.block_num}-${increment.timestamp}-${index}`}>
